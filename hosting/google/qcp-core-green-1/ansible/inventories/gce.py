...
When run in --list mode, instances are grouped by the following categories:
 ...
 - instance tags:
   ...
   the 'tag_foo' name.
 - instance labels:
   An entry is created for each label value.  For example, if you have one instance
   with labels {"env": "dev", "type": "webserver"}, second instance with labels {"env": "stage", "type": "webserver"},
   they will both be grouped together under the 'webserver' name.
...
class GceInventory(object):
    ...
    def get_config(self):
        ...
        cache_max_age = int(os.environ.get(
        ...
        for node in self.list_nodes():
            ...
            if labels is not None:
                for lab_key, lab_value in labels.items():
                    # Quantexa change
                    # We support multiple different group names derived from tags
                    label_list = [
                        'label_%s_%s' % (lab_key, lab_value),
                        '%s_%s' % (lab_key, lab_value),
                        lab_key,
                        lab_value
                    ]
            ...
            gce_md = meta["hostvars"][name]["gce_metadata"]
            is_dataproc = "dataproc-role" in gce_md
            is_preempt = node.extra['scheduling'].get('preemptible', False)

            if is_dataproc and not is_preempt:
                dp_role = "dataproc-" + gce_md["dataproc-role"].lower()
                if dp_role in groups:
                    groups[dp_role].append(name)
                else:
                    groups[dp_role] = [name]
