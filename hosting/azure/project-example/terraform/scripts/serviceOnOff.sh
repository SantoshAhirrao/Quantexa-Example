...
function usage() {
    ...
    echo -e "\n Terraform Service Variable: 'hdi-on', 'elastic-on', 'kibana-on' and 'aks-on'"
    echo -e "\n State: 0 to delete HDI cluster; 1 to turn create HDI cluster"
    echo -e "\n Terraform Target Module: 'module.hdinsight', 'module.elastic_no_loadbalancer' and 'module.azurerm_kubernetes_cluster'"
    echo -e "\n Sample Usage: ./serviceOnOff.sh hdi-on 0 \"module.hdinsight\""
    exit 1
}
...
if [[ "${machine}" = "Mac" ]]; then
   ...
   cd "$parent_path"/..
   sed -i '' 's|'${SERVICE}' = "[0-9]"|'${SERVICE}' = "'${STATE}'"|g' variable.tf
else
