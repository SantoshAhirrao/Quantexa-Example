...
case class CustomerRollupUnusualCounterpartyCountryForCustomerBatch(runDate: java.sql.Date)
  extends AugmentedDocumentScore[CustomerRollup[TransactionScoreOutput], CustomerScoreOutputWithUnderlying]
    with CustomerRollupUnusualCounterpartyCountryForCustomerTemplate


case class CustomerRollupUnusualCounterpartyCountryForCustomerOnDemand(runDate: java.sql.Date)
  extends AugmentedDocumentScoreWithLookup[Customer, CustomerRollup[TransactionScoreOutput], CustomerScoreOutputWithUnderlying]
    with CustomerRollupUnusualCounterpartyCountryForCustomerTemplate with HitAggregation {
...
trait CustomerRollupUnusualCounterpartyCountryForCustomerTemplate extends ScoreParameters {

  def runDate: java.sql.Date
  def id: String = "CR102_CustomerRollupUnusualCounterpartyCountryForCustomer"
  def name: String = "Unusual counter party country for customer"
  ...
  def score(document: CustomerRollup[TransactionScoreOutput])(implicit scoreInput: ScoreInput): Option[CustomerScoreOutputWithUnderlying] = {
    ...
    val triggeringScores = document.customScoreOutputMap
      .getOrElse(UnusualCounterpartyCountryForCustomer.id, Seq.empty).
    ...
    if (triggeringScores.isEmpty) None
      ...
      Some(CustomerScoreOutputWithUnderlying(
        ...
        description = Some(s"The customer has ${triggeringScores.size} transactions which were from ${unusualCountries.size} " +
          s"unusual counterparty countries: ${unusualCountries.take(maxNumberOfCountriesInDescription).mkString(", ")} " +
