package com.quantexa.example.etl.projects.fiu.postcodeprices
...
object ImportRawToParquet extends TypedSparkScript[ETLConfig] {
  ...
  def run(spark: SparkSession,
    ...
    val inputPath = etlConfig
      .scoringLookups
      .getOrElse(throw new IllegalStateException("No input path found in config. Please specify a value for 'scoringLookups.postcodePrices'."))
      .postcodePrices

    val reader = new ReaderWithSchemaValidation[PostcodePrice] {
