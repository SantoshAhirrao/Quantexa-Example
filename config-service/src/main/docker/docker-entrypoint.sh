...
echo "java -Dspring.profiles.active=$SPRING_PROFILES -Djava.security.egd=file:/dev/./urandom $KEYSTORE_PASSWORD_PROPERTY $TRUSTSTORE_PASSWORD_PROPERTY $OTHER_VM_ARGS_UNQUOTED -Dspring.cloud.config.server.overrides.quantexa.licence.path=quantexa-licence/quantexa.licence -jar /app.jar"
...
-Dspring.cloud.config.server.overrides.quantexa.licence.path=quantexa-licence/quantexa.licence \
-jar /app.jar
