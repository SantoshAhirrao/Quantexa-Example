...
if [ ! -z "$AKKA_MODE" ]; then
    AKKA_CONFIG_PROPERTY="-Dconfig.resource=/akka.$AKKA_MODE.conf"
fi
...
echo "java -Dspring.profiles.active=$SPRING_PROFILES -Djava.security.egd=file:/dev/./urandom $AKKA_CONFIG_PROPERTY $KEYSTORE_PASSWORD_PROPERTY $TRUSTSTORE_PASSWORD_PROPERTY $OTHER_VM_ARGS_UNQUOTED -jar /app.jar"
java -Dspring.profiles.active=$SPRING_PROFILES \
-Djava.security.egd=file:/dev/./urandom \
$AKKA_CONFIG_PROPERTY \
